...
if (typeof $argument !== "undefined") {
    let argStr = typeof $argument === "string" ? $argument : JSON.stringify($argument);
    if (argStr.includes("#")) isSilent = true;
}
...
function Checkin() {
    ...
    if (!cookie) {
        $.msg($.name + " ⚠️", "未找到 Cookie", "请确保 MITM 包含域名并访问网页登录 🚫");
        $.log("❌ 未找到 Cookie");
        $.done();
        return;
    }
        ...
        if (error) {
            ...
            try {
                ...
                if (obj.ret === 1) {
                    ...
                    $.log("⚠️ 签到结果: " + obj.msg);
                    const isChecked = obj.msg.indexOf("已经") !== -1;
                    // 如果不是因为“已经签到”，或者没有开启静默，就通知
                    if (!isChecked || !isSilent) {
                        $.msg($.name + " 🔔", "签到提示", `📝 信息: ${obj.msg}\n📅 日期: ${now.toLocaleDateString()}`);
                    }
                }
            } catch (e) {
                if (data && data.indexOf("html") !== -1) {
                    $.msg($.name + " 🛑", "Cookie 已失效", "请重新访问网页获取 🔄");
                    $.log("❌ Cookie 失效");
                } else {
