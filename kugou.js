...
(async () => {
  ...
  if (isPanel) {
    ...
    if (cachedContent) {
      $done({ title: "🎵 酷狗提现汇总", content: cachedContent, icon: "music.note.list", "icon-color": "#108ee9" });
    } else {
      $done({ title: "🎵 酷狗提现汇总", content: "⏳ 暂无缓存数据，请等待后台定时任务运行，或手动运行一次", icon: "music.note.list", "icon-color": "#108ee9" });
    }
    return; 
  }
