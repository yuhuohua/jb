...
const logKey = 'pingme_daily_log'; // 日志存储键
const SECRET = '0fOiukQq7jXZV2GRi9LGlO';
const MAX_VIDEO = 5;
const VIDEO_DELAY = 8000;
const SUMMARY_HOUR = 22; // 汇总通知时间点

// --- 智能化参数解析 ---
let NOTIFY_MODE = 1; // 默认每次通知
if (typeof $argument !== 'undefined') {
    if ($argument === "0" || $argument === 0) {
        NOTIFY_MODE = 0;
    } else if ($argument === "1" || $argument === 1) {
        NOTIFY_MODE = 1;
    } else if (typeof $argument === 'string' && $argument.includes('=')) {
        const argObj = Object.fromEntries($argument.split('&').map(item => item.split('=')));
        NOTIFY_MODE = parseInt(argObj.notify ?? argObj['通知模式'] ?? '1');
    } else if (typeof $argument === 'string') {
        const match = $argument.match(/[01]/);
        if (match) NOTIFY_MODE = parseInt(match[0]);
    }
}
...
async function startTasks() {
    ...
    console.log(`🔔 任务开始 (模式: ${NOTIFY_MODE === 0 ? "22点汇总" : "每次通知"})`);
    ...
    try {
        ...
        if (d.retcode === 0) msgs.push(`✅ 签到：成功`);
        ...
        if (vCount > 0) msgs.push(`🎬 视频：已完成 ${vCount} 次`);
        ...
        // 组装单次运行结果
        const runResult = `💰 ${oldBalance} ➔ ${newBalance} | ${msgs.join(' | ')}`;
        ...
        if (NOTIFY_MODE === 1) {
            // 实时通知模式
            await sendMsg(runResult, "");
            $.setdata(JSON.stringify(logData), logKey);
        } else {
            // 汇总通知模式 (0)
            if (now.getHours() >= SUMMARY_HOUR) {
                await sendMsg(`📊今日汇总\n${logData.summary.join('\n')}`, "");
                $.setdata("", logKey); // 发送后清空
                console.log("📤 已发送 22 点汇总通知并清空日志。");
            } else {
                $.setdata(JSON.stringify(logData), logKey);
                console.log(`📝 已记录运行日志，将在 ${SUMMARY_HOUR}:00 后汇总。`);
            }
        }

    } catch (err) {
        console.log(`❌ 运行异常: ${err}`);
        if (NOTIFY_MODE !== 0) await sendMsg("❌ 运行异常", String(err));
    }
}
